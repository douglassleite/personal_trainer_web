      ...
      <div class="max-w-2xl mx-auto">
        ...
        <div class="bg-white rounded-[2rem] shadow-2xl border border-gray-100 overflow-hidden">
          ...
          <div class="h-2 bg-gray-100">
            <div 
              class="h-full bg-gradient-to-r from-accent-500 to-accent-600 transition-all duration-500 ease-out"
          ...
          <div class="p-8 sm:p-12">
            ...
            <div class="text-center mb-10">
              <div class="inline-flex items-center justify-center w-20 h-20 bg-gradient-to-br from-accent-500 to-accent-600 rounded-3xl mb-5 shadow-lg">
              ...
              <div class="flex justify-center gap-2 mt-6">
                <div 
                  ...
                  :class="step <= currentStep ? 'w-8 bg-accent-500' : 'w-2 bg-gray-200'"
                />
              </div>
            </div>
            ...
            <div v-if="success" class="text-center py-12">
              ...
              <p class="text-gray-600 text-lg mb-10">
                Sua conta foi criada com sucesso. Baixe o app e comece sua transformação!
              </p>
              
              <div class="bg-gradient-to-br from-accent-50 to-primary-50 rounded-2xl p-8 max-w-md mx-auto">
                <div class="flex items-center justify-center gap-2 mb-6">
                  <Icon name="lucide:smartphone" class="w-6 h-6 text-accent-600" />
                ...
                <div class="mt-8 pt-6 border-t border-gray-200">
                  <p class="text-sm text-gray-600 mb-3">Quer ter seu próprio Personal Trainer?</p>
                  <NuxtLink 
                    to="/cadastro/personal"
                    class="inline-flex items-center gap-2 text-primary-600 hover:text-primary-700 font-semibold text-sm"
                  >
            ...
            <form v-else @submit.prevent="handleNext" class="space-y-6">
              ...
              <div v-show="currentStep === 2" class="space-y-6">
                <div>
                  <label class="block text-sm font-semibold text-gray-700 mb-3">Qual seu objetivo principal?</label>
                  <div class="grid sm:grid-cols-2 gap-3">
                    <label 
                      ...
                      :class="form.objetivo === objetivo.value ? 'border-accent-500 bg-accent-50' : 'border-gray-200 hover:border-gray-300'"
                    >
                      <input 
                        type="radio" 
                        ...
                        class="sr-only"
                      />
                      <Icon :name="objetivo.icon" class="w-8 h-8 mb-2" :class="form.objetivo === objetivo.value ? 'text-accent-600' : 'text-gray-500'" />
                      <span class="font-medium text-center" :class="form.objetivo === objetivo.value ? 'text-accent-700' : 'text-gray-700'">
                        {{ objetivo.label }}
                      </span>
                      <Icon v-if="form.objetivo === objetivo.value" name="lucide:check-circle" class="absolute top-2 right-2 w-5 h-5 text-accent-600" />
                    </label>
                  </div>
                </div>

                <div>
                  <label class="block text-sm font-semibold text-gray-700 mb-3">Nível de experiência</label>
                  <div class="grid grid-cols-3 gap-3">
                    <label 
                      v-for="nivel in niveis" 
                      :key="nivel.value"
                      class="relative flex flex-col items-center p-4 border-2 rounded-xl cursor-pointer transition-all"
                      :class="form.nivelExperiencia === nivel.value ? 'border-accent-500 bg-accent-50' : 'border-gray-200 hover:border-gray-300'"
                    >
                      <input 
                        type="radio" 
                        ...
                        class="sr-only"
                      />
                      <Icon :name="nivel.icon" class="w-6 h-6 mb-2" :class="form.nivelExperiencia === nivel.value ? 'text-accent-600' : 'text-gray-500'" />
                      <span class="font-medium text-center text-sm" :class="form.nivelExperiencia === nivel.value ? 'text-accent-700' : 'text-gray-700'">
                        {{ nivel.label }}
                      </span>
                    </label>
                  </div>
                ...
                <div class="bg-gradient-to-br from-accent-50 to-primary-50 rounded-xl p-5">
                  <div class="flex gap-3">
                    <Icon name="lucide:info" class="w-5 h-5 text-accent-600 flex-shrink-0 mt-0.5" />
                    <div class="text-sm text-gray-700">
                      <p class="font-semibold mb-1">Essas informações ajudam seu Personal</p>
                      <p>Com seus objetivos e nível, seu Personal poderá criar treinos mais eficientes para você.</p>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div v-show="currentStep === 3" class="space-y-6">
                <div>
                  ...
                  <div class="relative">
                    <Icon name="lucide:lock" class="absolute left-4 top-1/2 transform -translate-y-1/2 w-5 h-5 text-gray-400 z-10 transition-colors" :class="{ 'text-accent-500': form.password }" />
                    <input 
                      v-model="form.password"
                      :type="showPassword ? 'text' : 'password'"
                      placeholder="Mínimo 6 caracteres"
                      class="w-full pl-12 pr-14 py-3.5 border-2 border-gray-200 rounded-xl focus:ring-2 focus:ring-accent-500 focus:border-accent-500 transition-all"
                ...
                <div class="bg-gradient-to-br from-accent-50 to-primary-50 rounded-xl p-5">
                  <div class="flex items-start gap-3">
                    <input 
                      ...
                      type="checkbox"
                      class="mt-1 w-5 h-5 text-accent-600 border-2 border-gray-300 rounded focus:ring-2 focus:ring-accent-500"
                    />
                    <label class="text-sm text-gray-700">
                      Li e aceito os <a href="#" class="text-accent-600 hover:text-accent-700 font-semibold">Termos de Uso</a> e 
                      <a href="#" class="text-accent-600 hover:text-accent-700 font-semibold">Política de Privacidade</a>
                    </label>
                  </div>
              ...
              <div class="flex gap-4 pt-4">
                ...
                <button
                  type="submit"
                  :disabled="loading"
                  class="flex-1 px-6 py-3.5 bg-gradient-to-r from-accent-500 to-accent-600 hover:from-accent-600 hover:to-accent-700 text-white rounded-xl transition-all font-semibold shadow-lg hover:shadow-xl disabled:opacity-50 disabled:cursor-not-allowed"
                >
        ...
        <div class="text-center mt-8">
          <p class="text-gray-600">
            Já tem uma conta? 
            <NuxtLink to="/login" class="text-accent-600 hover:text-accent-700 font-semibold">
              Fazer login
            </NuxtLink>
          </p>
        </div>
...
const objetivos = [
  { value: 'perder_peso', label: 'Perder Peso', icon: 'lucide:scale' },
  { value: 'ganhar_massa', label: 'Ganhar Massa', icon: 'lucide:dumbbell' },
  { value: 'definir', label: 'Definir Corpo', icon: 'lucide:activity' },
  { value: 'saude', label: 'Saúde Geral', icon: 'lucide:heart-pulse' }
]

const niveis = [
  { value: 'iniciante', label: 'Iniciante', icon: 'lucide:sprout' },
  { value: 'intermediario', label: 'Intermediário', icon: 'lucide:trending-up' },
  { value: 'avancado', label: 'Avançado', icon: 'lucide:flame' }
]
...
const validateStep = () => {
  ...
  switch (currentStep.value) {
    case 1:
      ...
      break
    case 2:
      // Objetivos são opcionais mas têm valores padrão
      isValid = true
      break
    case 3:
...
useHead({
  title: 'Cadastro Aluno - Academia na Mão'
})
</script>
